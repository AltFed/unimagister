...
function [possib,sp,r,Terminal]=grid3(possib,s,a)
Terminal=false;
...
won=checker(possib,s,colore,ins);
...
elseif( won == 0 && ~nz) % stato terminale tie game
    ...
    if( won ~= 1 && nz) % se il giocatore 1 vince non posso giocare
        for i=1:1:6
            if(sp(i,a)==0 && i==6) %verifica dell'ultima linea
                sp(i,a)=colore;
                ins=[i,a]; %salvo il punto in cui ho inserito per effettuare le verifiche dopo
                break;
            end

            if(sp(i,a)~=0) %verifica valida fino alla penultima riga
                if(possib(a)==1)
                    sp(i-1,a)=colore;
                    ins=[i-1,a];
                    if(i-1==1)
